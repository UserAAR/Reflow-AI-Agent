...
export function Security() {
  return (
    <section id="security" className="py-16 sm:py-20 lg:py-28 border-t border-black/[0.06] dark:border-white/[0.08]">
      <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-[1fr_1.2fr] gap-10 lg:gap-16">
        <div>
          <div className="text-emerald-700" style={{ fontSize: 12, fontWeight: 600, letterSpacing: "0.12em" }}>ENTERPRISE TRUST</div>
          ...
          <p className="mt-4 text-neutral-600 dark:text-neutral-400 max-w-md" style={{ fontSize: "clamp(14px,1.1vw,16px)", lineHeight: 1.6 }}>
            Reflow is built so leadership, audit and operations teams can move quickly without losing control over what the system decides on their behalf.
          </p>
          <div className="mt-6 sm:mt-8 inline-flex items-center gap-3 rounded-xl border border-black/[0.06] dark:border-white/[0.08] bg-white dark:bg-neutral-900 px-4 py-3 shadow-[0_8px_24px_-16px_rgba(0,0,0,0.15)]">
        ...
        <div className="grid sm:grid-cols-2 gap-px bg-black/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-2xl overflow-hidden">
          {items.map((it) => (
            <div key={it.title} className="bg-white dark:bg-neutral-900 p-5 sm:p-6">
              <div className="w-9 h-9 rounded-xl bg-neutral-50 dark:bg-neutral-900 text-neutral-700 dark:text-neutral-300 border border-black/[0.06] dark:border-white/[0.08] flex items-center justify-center">{it.icon}</div>
              <div className="mt-4 text-neutral-950 dark:text-white" style={{ fontSize: 15, fontWeight: 550 }}>{it.title}</div>
              <div className="mt-1.5 text-neutral-600 dark:text-neutral-400" style={{ fontSize: 13.5, lineHeight: 1.55 }}>{it.body}</div>
            </div>
          ))}
        </div>
      </div>
