...
export function WhyReflow() {
  return (
    <section id="product" className="py-16 sm:py-20 lg:py-28 border-t border-black/[0.06] dark:border-white/[0.08]">
      <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-10 sm:mt-14 grid sm:grid-cols-2 lg:grid-cols-3 gap-px bg-black/[0.06] border border-black/[0.06] dark:border-white/[0.08] rounded-2xl overflow-hidden">
          {items.map((it) => (
            <div key={it.title} className="bg-white dark:bg-neutral-900 p-5 sm:p-7 hover:bg-neutral-50 dark:hover:bg-neutral-800/40 transition-colors">
              <div className="w-9 h-9 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center border border-emerald-100">
                {it.icon}
              </div>
              <div className="mt-4 sm:mt-5 text-neutral-950 dark:text-white" style={{ fontSize: "clamp(14px, 1.1vw, 16px)", fontWeight: 550, letterSpacing: "-0.01em" }}>
                {it.title}
              </div>
              <div className="mt-2 text-neutral-600 dark:text-neutral-400" style={{ fontSize: "clamp(13px, 1vw, 14px)", lineHeight: 1.55 }}>
                {it.body}
              </div>
            </div>
          ))}
